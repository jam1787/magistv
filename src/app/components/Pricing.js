...
const Pricing = () => {
  return (
    <section id="pricing">
      <h2 className="mx-auto text-5xl font-semibold text-center mt-20 md:mt-0 md:mb-20">Elige tu plan</h2>
      <div className="flex justify-center items-center flex-wrap text-center text-balance">
        {pricingData.map(({plan, time, price, description}, i) => 
          <div key={i} className={`w-fit flex flex-col m-10 md:mx-6 ${i === 1 && 'w-fit bg-gradient-to-b from-[#0024d6] 60% to-[#0101301b] 100% p-1 md:-translate-y-4'}`}>
            <div className={`h-[20rem] flex flex-col justify-between px-2 py-5 md:px-4 max-w-72 ${i === 1 && 'afakhshha bg-gradient-to-b from-[#01043b] to-[#01013000] pt-8'}`}>
              ...
              <LinkComponent text='CONTRATAR' classStyle={`${i === 1 && 'bg-[#002afe] text-white'} block py-3 px-10 w-full text-md transtion hover:opacity-85`} />
            </div>
          </div>
        )}
      </div>
